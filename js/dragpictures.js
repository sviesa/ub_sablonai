$(function() {
  ...
  $( ".to" ).on( "sortreceive", function(event, ui) {
    if( !( $(this).attr("type") ===  ui.item.attr("type")) ){
      ...
      $.each($($(this).parent()[0]).find('.circle'), function(i, value){
        // console.log( $(value) );
        $(value).animate({"background-color": "red"}, 200, function() {
          $(value).animate({"background-color": "white"})
        });
      });
    }
